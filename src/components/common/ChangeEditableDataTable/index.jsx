 ...
 * @author Rex.Guo
 */
class ChangeEditableDataTable extends React.Component {
  constructor(props) {
    super(props);
  ...
  onDeleteItems = () => {
    ...
    //当明细行付款申请表字段不为空时，此明细行不允许删除
    let isCanDelete = true;
    try {
      selectedRows.forEach(item => {
        if (item.paymentNo !== null && item.id > 0) {
          isCanDelete = false;
          throw Error();
        }
      });
    ...
    if (isCanDelete) {
      ...
      createMessage({
        type: 'error',
        description: '当明细行付款申请表字段不为空时，此付款计划不允许删除',
      });
    }
  };
...
ChangeEditableDataTable.propTypes = {
...
ChangeEditableDataTable.defaultProps = {
  buttons: [],
  readOnly: false,
  onAdd: undefined,
...
export default ChangeEditableDataTable;
