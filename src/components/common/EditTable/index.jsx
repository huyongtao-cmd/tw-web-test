...
import { Button, Col, Popconfirm, Row, Table, Form } from 'antd';
import { equals, type, isNil, isEmpty } from 'ramda';
import styles from './styles.less';
import { getGuid } from '@/utils/stringUtils';

const getRowClassName = (record, index) => (index % 2 === 0 ? styles['table-stripe-odd'] : '');
...
class EditableDataTable extends React.Component {
  ...
  onTableDoubleClick = async (record, index) => {
    const { dataTable } = this.props;
    dataTable(true, record, index);
  };
      ...
      <div style={{ backgroundColor: 'white', borderRadius: 5 }}>
        ...
        <div style={{ padding: !isEmpty(leftButtons) ? '10px 24px' : '0' }}>
          <Table
            ...
            onRow={(record, index) => {
              return {
                onDoubleClick: () => this.onTableDoubleClick(record, index),
              };
            }}
            {...tableProps}
          />
