...
export default class VDraggable extends Component {
  ...
  // 生成拖拽组件
  createDraggleComponent(data, sortKey, style, uId, render, codeKey, modal, found) {
    return (
      <>
        {data.sort(this.compare(sortKey)).map(item => {
          return (
            <div
        ...
        {found && (
          <div className={styles.custom} onClick={modal}>
            <div>
              <i />
            </div>
            <span>自定义入口</span>
  ...
  render() {
    const { value, sortKey, codeKey, style, render, modal, found } = this.props;
    const { uId } = this.state;
    return (
      <div>
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
          {this.createDraggleComponent(value, sortKey, style, uId, render, codeKey, modal, found)}
        </div>
      </div>
    );
  }
