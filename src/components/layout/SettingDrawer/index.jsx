...
@connect(({ uiSettings }) => ({ setting: uiSettings }))
class SettingDrawer extends PureComponent {
  ...
  render() {
    const { setting } = this.props;
    const { navTheme, primaryColor, layout, colorWeak, themeEnabled } = setting;
    const { collapse } = this.state;
    if (!themeEnabled) {
      return <></>;
    }
    return (
      ...
      >
        <div className={styles.content}>
          ...
          <Divider />
          <CopyToClipboard
            text={JSON.stringify(omit(['colorWeak'], setting), null, 2)}
