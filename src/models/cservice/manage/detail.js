  ...
  effects: {
    ...
    *queryDetail({ payload }, { call, put, select }) {
      ...
      if (mode === 'create') {
        ...
        const { id } = payload;
        const { status, response } = yield call(getManageDetailById, id);
        if (status === 100) {
          // 主动取消请求
          return {};
        }
    ...
    *save({ payload }, { call, put, select }) {
      ...
      if (mode === 'create') {
        const { status, response } = yield call(createCalendarDetail, values);
        if (status === 100) {
          // 主动取消请求
          return {};
        }
      ...
      if (mode === 'copy') {
        ...
        const { status, response } = yield call(createCalendarDetail, valuesNew);
        if (status === 100) {
          // 主动取消请求
          return {};
        }
      ...
      if (mode === 'edit') {
        const { id } = payload;
        const { status, response } = yield call(createCalendarDetail, { id, ...values });
        if (status === 100) {
          // 主动取消请求
          return {};
        }
