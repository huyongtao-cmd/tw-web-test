...
import { getCalendarList, deleteCalendar, generateDetail } from '@/services/cservice/manage';
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { status, response } = yield call(getCalendarList, params);
      if (status === 200) {
    ...
    *delete({ payload }, { call, put, select }) {
      ...
      const { status, response } = yield call(deleteCalendar, payload);
      if (status === 200) {
