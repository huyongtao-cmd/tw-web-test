...
import { deleteRegularList, getViewRegularList } from '@/services/cservice/regularCare';
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      const { id } = payload;
      const { status, response } = yield call(getViewRegularList, id);
      if (status === 200) {
    ...
    *delete({ payload }, { call, put, select }) {
      const { searchForm } = yield select(({ viewRegularList }) => viewRegularList);
      const { status, response } = yield call(deleteRegularList, payload);
