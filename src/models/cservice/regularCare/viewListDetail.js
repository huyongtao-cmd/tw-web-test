...
import { createRegularListDetail, getRegularListDetailById } from '@/services/cservice/regularCare';
import { findResById } from '@/services/plat/computer';

export default {
  namespace: 'viewRegularListDetail',
  state: {
    formData: {},
  },
  ...
  effects: {
    ...
    *queryDetail({ payload }, { call, put, select }) {
      ...
      if (mode === 'create') {
        ...
        const { id } = payload;
        const { status, response } = yield call(getRegularListDetailById, id);
        if (status === 100) {
          // 主动取消请求
          return {};
        }
    ...
    *save({ payload }, { call, put, select }) {
      ...
      if (mode === 'create') {
        const { status, response } = yield call(createRegularListDetail, values);
        if (status === 100) {
          // 主动取消请求
          return {};
        }
      ...
      if (mode === 'edit') {
        const { id } = payload;
        const { status, response } = yield call(createRegularListDetail, { id, ...values });
        if (status === 100) {
          // 主动取消请求
          return {};
        }
