...
export default {
  namespace: 'buReimbursementCreate',
  state: {
    formData: {},
    sharingData: {}, // 分摊信息
    detailList: [], // 分摊列表
  ...
  effects: {
    ...
    *query({ payload }, { call, put }) {
      ...
      if (mode === 'create') {
        const { resId } = payload;
        const { status, response } = yield call(findExpenseById, id);
        if (status === 200) {
          ...
          yield put({
            ...
            payload: {
              ...
              sharingData: {
                reimAmt: datum.taxedReimAmt,
                applicantTime: moment().format('YYYY-MM-DD'),
                sharingStatus: 'CREATE',
                applicantResId: resId,
              },
            },
          });
          return response;
