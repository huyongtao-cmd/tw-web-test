...
import {
  getDistInfoByReact,
  saveDistInfo,
  getPeriodInfoByDateRq,
} from '@/services/user/Contract/recvplan';
import createMessage from '@/components/core/AlertMessage';
import { closeThenGoto } from '@/layouts/routerControl';

export default {
  namespace: 'distInfoPlat',
  state: {
    formData: {},
    dataSource: [],
    total: 0,
  ...
  effects: {
    ...
    *getPeriodInfoByDate({ payload }, { call, put }) {
      const { status, response } = yield call(getPeriodInfoByDateRq, payload);
      if (status === 200) {
        if (response.ok) {
          yield put({
            type: 'updateForm',
            payload: {
              finPeriodId:
                Array.isArray(response.datum) && !isEmpty(response.datum)
                  ? response.datum[0].id
                  : undefined,
            },
          });
        } else {
          createMessage({ type: 'warn', description: response.reason || '获取收入核算期间失败' });
        }
      }
    },
