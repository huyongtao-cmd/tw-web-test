...
import {
  customSettingCreateRq,
  customSettingModifyRq,
  customSettingDetailRq,
...
export default {
  ...
  effects: {
    *init({ payload }, { call, put }) {
      ...
      const { data } = yield outputHandle(customSettingDetailRq, { id });
      yield put({
        type: 'updateState',
        payload: {
          formData: data,
    ...
    *query({ payload }, { call, put }) {
      ...
      const { status, response } = yield call(customSettingListPagingRq, param);
      if (status === 200) {
    ...
    *queryValueData({ payload }, { call, put }) {
      const { status, response } = yield call(customSettingDetailRq, payload);
      if (status === 200) {
    ...
    *save({ payload }, { call, put, select }) {
      ...
      if (id && id > 0) {
        // 编辑
        const { data } = yield outputHandle(customSettingModifyRq, formData);
        output = data;
      } else {
        // 新增
        const { data } = yield outputHandle(customSettingCreateRq, formData);
    ...
    *delete({ payload }, { call, put, select }) {
      const { searchForm } = yield select(({ customSettingIndex }) => customSettingIndex);
      const { status, response } = yield call(customSettingLogicalDeleteRq, payload);
      if (status === 200) {
        if (response && response.ok) {
