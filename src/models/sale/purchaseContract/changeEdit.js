...
export default {
  namespace: 'salePurchaseChangeEdit',
  state: {
  ...
  effects: {
    ...
    *save({ payload }, { call, put, select }) {
      let defkey = '';
      const { response } = yield call(purchaseChangeSave, payload);
      if (response && response.ok) {
    ...
    *queryUdcList({ payload }, { call, put }) {
      const { response } = yield call(udcSelect, payload);
      yield put({
        type: 'updateState',
        payload: {
          platTypeArr: response || [],
        },
      });
    },
    ...
    *getPageConfig({ payload }, { call, put, select }) {
      ...
      const { formData } = yield select(({ salePurchaseChangeEdit }) => salePurchaseChangeEdit);
      if (status === 200) {
        yield put({
          type: 'updateState',
          payload: {
