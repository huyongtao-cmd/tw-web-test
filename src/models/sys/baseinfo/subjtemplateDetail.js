  ...
  effects: {
    ...
    *save(_, { call, select }) {
      const { formData, dataSource, deleteList } = yield select(
        ({ sysSubjtempDetail }) => sysSubjtempDetail
      );
      ...
      if (formData.id) {
        ...
        if (response && response.ok) {
          if (response.datum.errorCode) {
            ...
            createMessage({ type: 'success', description: '保存成功' });
            closeThenGoto(`/plat/finAccout/subjtempdetail?id=${response.datum.id}&mode=update`);
            // router.push(`/plat/finAccout/subjtempdetail?id=${response.datum.id}&mode=update`);
    ...
    *saveAccMasTree({ payload }, { put, select }) {
      ...
      const { dataSource, formData } = yield select(({ sysSubjtempDetail }) => sysSubjtempDetail);
      // modalData重构列表数据
