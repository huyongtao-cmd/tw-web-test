...
export default {
  namespace: 'messageConfigPersonalShielding',
  state: {
    list: [],
    total: 0,
    ...
    *delete({ payload }, { call, put, select }) {
      ...
      if (status === 200 && response.ok) {
        ...
        const { searchForm } = yield select(
          ({ messageConfigPersonalShielding }) => messageConfigPersonalShielding
        );
        yield put({
          type: 'query',
          payload: {
    ...
    *save({ payload }, { call, put, select }) {
      ...
      if (status === 200) {
        if (response && response.ok) {
          ...
          closeThenGoto(`/user/center/message/personalShielding`);
          yield put({
            type: 'query',
            payload: {
              ...searchForm,
