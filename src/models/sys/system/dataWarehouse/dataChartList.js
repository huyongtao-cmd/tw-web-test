...
import { dataChartListPaging, dataChartLogicalDelete } from '@/services/sys/system/dataWarehouse';
...
export default {
  namespace: 'dataChartList',
  state: {
    searchForm: defaultSearchForm,
    dataSource: [],
    total: undefined,
    dataChartMainModalVisible: false,
  },
  effects: {
    *query({ payload }, { call, put }) {
      const param = handleEmptyProps(payload);
      const { response, status } = yield call(dataChartListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(dataChartLogicalDelete, payload);
      if (status === 200) {
