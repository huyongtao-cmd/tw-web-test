...
import {
  dataExtractListPaging,
  dataExtractLogicalDelete,
  dataExtractRun,
...
export default {
  namespace: 'dataExtractList',
  state: {
    searchForm: defaultSearchForm,
    dataSource: [],
    total: undefined,
    dataExtractMainModalVisible: false,
  },
  effects: {
    *query({ payload }, { call, put }) {
      const param = handleEmptyProps(payload);
      const { response, status } = yield call(dataExtractListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(dataExtractLogicalDelete, payload);
      if (status === 200) {
    ...
    *dataExtractRun({ payload }, { call, put, select }) {
      const { response, status } = yield call(dataExtractRun, payload);
      if (status === 200) {
        createMessage({ type: 'success', description: '成功' });
      } else {
        createMessage({ type: 'warn', description: response.reason || '失败' });
