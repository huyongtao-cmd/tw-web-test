...
import {
  helpDirectoryListPaging,
  helpDirectoryLogicalDelete,
  helpDirectoryTree,
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { response, status } = yield call(helpDirectoryListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(helpDirectoryLogicalDelete, payload);
      if (status === 200) {
        createMessage({ type: 'success', description: '成功' });
        const { searchForm } = yield select(({ helpDirectoryList }) => helpDirectoryList);
