import { queryExpenseAccount, saveAccount } from '@/services/user/expense/expense';
import createMessage from '@/components/core/AlertMessage';
import moment from 'moment';

export default {
  namespace: 'userExpenseAccountList',
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const {
        response: { rows, total },
      } = yield call(queryExpenseAccount, newPayload);
      yield put({
        type: 'updateState',
        payload: {
    ...
    *saveAccount({ payload }, { call, select, put }) {
      const { accPayBatchIds, jdeAccount } = yield select(
        ({ userExpenseAccountList }) => userExpenseAccountList
      );
      if (!accPayBatchIds || accPayBatchIds.length === 0) {
        createMessage({ type: 'warn', description: '请先选择记录！' });
        return {};
      }
      const { state, response } = yield call(saveAccount, {
