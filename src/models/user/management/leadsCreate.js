...
export default {
  ...
  effects: {
    ...
    *saveLead({ payload }, { put, call, select }) {
      const { formData, page } = yield select(({ userLeadsCreate }) => userLeadsCreate);
    ...
    *submit({ payload }, { put, call, select }) {
      const { status, response } = yield call(create, { ...payload.formData, submitted: true });
      if (status === 100) {
        // 主动取消请求
        return;
      }
