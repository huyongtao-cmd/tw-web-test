...
export default {
  namespace: 'probationLast',

  state: {
    formData: {},
    resData: [],
    baseBuData: [],
    resultChkList: [], // 入职培训检查事项
    flowForm: {
      remark: undefined,
      dirty: false,
    },
  ...
  effects: {
    ...
    *checkresult({ payload }, { call, put }) {
      ...
      if (status === 200 && response.ok) {
        ...
        createMessage({ type: 'error', description: response.reason || '获取检查事项列表失败' });
      }
    },
    ...
    *createSubmit({ payload }, { call, put, select }) {
      const { formData } = yield select(({ probationLast }) => probationLast);
      const { id, ...newParams } = formData;

      const { status, response } = yield call(saveFinalRq, { ...newParams, ...payload });
      if (status === 100) {
        // 主动取消请求
        return {};
      }
    ...
    *submit({ payload }, { call, put, select }) {
      const {
        ...
        capacityListSelected,
        capacityListSelectedDelId,
      } = yield select(({ probationLast }) => probationLast);
      ...
      const { status, response } = yield call(saveFinalRq, { ...formData, ...payload });
      if (status === 100) {
        // 主动取消请求
        return {};
      }
    ...
    *getResults({ payload }, { call, put }) {
      const { status, response } = yield call(getResultsRq, payload);
      if (status === 100) {
        // 主动取消请求
        return;
      }
