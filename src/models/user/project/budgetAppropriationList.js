...
export default {
  namespace: 'budgetAppropriationList',
  state: {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { response, status } = yield call(budgetAppropriationListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(budgetAppropriationLogicalDelete, payload);
      if (status === 200) {
