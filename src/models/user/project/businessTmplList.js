...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { response, status } = yield call(templateResPlanningListUri, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(templateResPlanningDeleteUri, payload);
      if (status === 200) {
        createMessage({ type: 'success', description: '删除成功' });
        const { searchForm } = yield select(({ businessTmplList }) => businessTmplList);
