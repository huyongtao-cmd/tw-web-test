...
import { findProjLedger, findProjLedgerIo } from '@/services/user/project/project';
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      const param = fromQs();
      const { response: ledgerData } = yield call(findProjLedger, { projId: param.projId });
      const { response } = yield call(findProjLedgerIo, { projId: param.projId, ...payload });
      // 合计表格数据
      ...
      yield put({
        ...
        payload: {
          ...
          total: list[0] !== null ? response.total : 0,
        },
      });
    },
  },
