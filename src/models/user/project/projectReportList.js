...
import {
  queryBriefInfoListPaging,
  logicalDeleteBriefInfo,
  projectLaborUploadHandle,
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { response, status } = yield call(queryBriefInfoListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(logicalDeleteBriefInfo, payload);
      if (status === 200) {
    ...
    *uploadLabor({ payload }, { call, put, select }) {
      const { response, status } = yield call(projectLaborUploadHandle, payload);
