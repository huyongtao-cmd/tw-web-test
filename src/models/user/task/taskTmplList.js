...
import { taskTmplListPaging, taskTmplLogicalDelete } from '@/services/user/task/task';
...
export default {
  ...
  effects: {
    *query({ payload }, { call, put }) {
      ...
      const { response, status } = yield call(taskTmplListPaging, param);
    ...
    *delete({ payload }, { call, put, select }) {
      const { response, status } = yield call(taskTmplLogicalDelete, payload);
      if (status === 200) {
