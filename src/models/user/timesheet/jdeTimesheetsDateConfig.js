...
import { queryJdeExportListMethod, saveJdeExportMethod } from '@/services/sys/baseinfo/vacation';
...
export default {
  namespace: 'jdeTimesheetsDateConfig',
  state: {
    year: moment(),
  ...
  effects: {
    *query({ payload }, { call, put }) {
      const { response } = yield call(queryJdeExportListMethod, payload.year.get('y'));
    ...
    *save({ payload }, { call, select }) {
      const { datas } = yield select(({ jdeTimesheetsDateConfig }) => jdeTimesheetsDateConfig);
      const { response } = yield call(saveJdeExportMethod, datas);
