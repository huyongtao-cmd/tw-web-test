  ...
  effects: {
    ...
    *projectPlanEdit({ payload }, { call, put, select }) {
      ...
      if (!fromQs().id) {
        ...
        if (data.id) {
          const { id } = data;
          const { scene, projectId, planType } = fromQs();
          if (window.location.href.includes('sessionMgmt')) {
            router.push(
              `/workTable/projectMgmt/sessionMgmt/edit?id=${id}&projectId=${projectId}&scene=${scene}&planType=${planType}&mode=EDIT`
            );
          } else {
            router.push(
              `/workTable/projectMgmt/planList/edit?id=${id}&projectId=${projectId}&scene=${scene}&planType=${planType}&mode=EDIT`
            );
          }
        } else {
