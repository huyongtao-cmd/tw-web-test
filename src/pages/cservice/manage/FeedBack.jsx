...
@connect(({ loading, feedBack, dispatch, user }) => ({
  loading,
  feedBack,
  dispatch,
  user: user.user,
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card
          ...
          style={{ marginTop: '6px' }}
          title={<Title icon="profile" text="运维日历循环事项明细反馈" />}
          bordered={false}
        >
          {!queryBtn ? (
            <>
              ...
              <FieldList layout="horizontal" getFieldDecorator={getFieldDecorator} col={2}>
                ...
                >
                  <FileManagerEnhance
                    api="/api/op/v1/omCalendar/operation/feedback/sfs/token"
                    dataKey={formData.id}
                    listType="text"
                    disabled={false}
                  />
