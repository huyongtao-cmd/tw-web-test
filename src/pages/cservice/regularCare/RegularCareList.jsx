...
const DOMAIN = 'regularCareList';
const { RangePicker } = DatePicker;
const particularColumns = [
  { dataIndex: 'code', title: '编号', span: 16 },
  { dataIndex: 'name', title: '名称', span: 16 },
];
@connect(({ loading, regularCareList }) => ({
  regularCareList,
  loading: loading.effects[`${DOMAIN}/query`],
}))
@mountToTab()
class RegularCareList extends PureComponent {
  componentDidMount() {
  ...
  render() {
    const {
      loading,
      dispatch,
      regularCareList: { list = [], total = 0, searchForm },
    } = this.props;
    const tableProps = {
      rowKey: 'id',
      columnsCache: DOMAIN,
      ...
      leftButtons: [
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            router.push(`/cservice/manage/regularCareDetail?mode=create`);
          },
        },
        {
          key: 'edit',
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            ...
            router.push(`/cservice/manage/regularCareDetail?mode=edit&configId=${id}`);
          },
        },
        {
          key: 'edit',
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            const { id } = selectedRows[0];
            router.push(`/cservice/manage/regularCareDetailCopy?mode=copy&configId=${id}`);
          },
        },
        {
          key: 'remove',
        ...
        },
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            ...
            if (errorFlag) {
              ...
              const { id } = selectedRows[0];
              router.push(`/cservice/manage/viewRegularList?configId=${id}`);
            }
          },
        },
      ],
...
export default RegularCareList;
