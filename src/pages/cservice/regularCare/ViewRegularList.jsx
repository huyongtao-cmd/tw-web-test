...
const DOMAIN = 'viewRegularList';
const { RangePicker } = DatePicker;
const particularColumns = [
  { dataIndex: 'code', title: '编号', span: 16 },
  { dataIndex: 'name', title: '名称', span: 16 },
];
@connect(({ loading, viewRegularList }) => ({
  viewRegularList,
  loading: loading.effects[`${DOMAIN}/query`],
}))
@mountToTab()
class ViewRegularList extends PureComponent {
  componentDidMount() {
  ...
  render() {
    ...
    const {
      loading,
      dispatch,
      viewRegularList: { list = [], total = 0, searchForm },
    } = this.props;
    const tableProps = {
      rowKey: 'id',
      columnsCache: DOMAIN,
      ...
      leftButtons: [
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            router.push(`/cservice/manage/viewRegularListDetail?mode=create&configId=${configId}`);
          },
        },
        {
          key: 'edit',
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            ...
            router.push(
              `/cservice/manage/viewRegularListDetail?mode=edit&listId=${id}&configId=${configId}`
            );
          },
        },
        {
        ...
        },
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            const { id } = selectedRows[0];
            router.push(`/cservice/manage/feedRegularBack?detailId=${id}&configId=${configId}`);
          },
        },
      ],
    };
    ...
    return (
      <PageHeaderWrapper title="客户定期关怀查询">
...
export default ViewRegularList;
