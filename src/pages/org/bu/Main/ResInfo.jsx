...
const DOMAIN = 'orgBuResInfo';

@connect(({ dispatch, loading, orgbu, orgBuResInfo }) => ({
  dispatch,
  orgbu,
  loading,
  orgBuResInfo,
    ...
    const fields = [
      ...
      <Field
        ...
        decorator={{
          ...
          rules: [
            {
              required: !!pageFieldJson.coopType.requiredFlag,
              message: '必填',
            },
          ],
        }}
      ...
      mode === 'create' ? (
        <Field
          ...
          decorator={{
            ...
            rules: [
              {
                required: !!pageFieldJson.eqvaRatio.requiredFlag,
                message: '必填',
              },
            ],
          }}
        >
          <InputNumber
            style={{ width: '100%' }}
            min={0}
            placeholder={`请输入${pageFieldJson.eqvaRatio.displayName}`}
            disabled
          />
        </Field>
      ) : (
      ...
      <Field
        name="salaryPeriod"
        key="salaryPeriod"
        label={pageFieldJson.salaryPeriod.displayName}
        sortNo={pageFieldJson.salaryPeriod.sortNo}
        decorator={{
          initialValue: formData.salaryPeriod,
          rules: [
            {
              required: !!pageFieldJson.salaryPeriod.requiredFlag,
              message: '必填',
            },
          ],
        }}
      >
        <UdcSelect
          ...
          placeholder={`请选择${pageFieldJson.salaryPeriod.displayName}`}
        />
      </Field>,
      ...
      <Field
        name="salaryMethod"
        key="salaryMethod"
        label={pageFieldJson.salaryMethod.displayName}
        sortNo={pageFieldJson.salaryMethod.sortNo}
        decorator={{
          initialValue: formData.salaryMethod,
          rules: [
            {
              required: !!pageFieldJson.salaryMethod.requiredFlag,
              message: '必填',
            },
          ],
        }}
      >
        <UdcSelect
          ...
          placeholder={`请选择${pageFieldJson.salaryMethod.displayName}`}
        />
      </Field>,
      <Field
        name="remark"
  ...
  render() {
    ...
    const listTableProps = {
      ...
      columns: [
        ...
        buRoleName.visibleFlag && {
          title: `${buRoleName.displayName}`,
          sortNo: `${buRoleName.sortNo}`,
          dataIndex: 'roleName',
          width: 180,
          align: 'center',
        },
        ...
        eqvaRatio.visibleFlag && {
          title: `${eqvaRatio.displayName}`,
        ...
        salaryPeriod.visibleFlag && {
          title: `${salaryPeriod.displayName}`,
          sortNo: `${salaryPeriod.sortNo}`,
          dataIndex: 'salaryPeriodDesc',
          width: 80,
          align: 'center',
        },
        salaryMethod.visibleFlag && {
          title: `${salaryMethod.displayName}`,
          sortNo: `${salaryMethod.sortNo}`,
        ...
        eqvaRatioDetail.visibleFlag && {
          title: eqvaRatioDetail.displayName || '详情',
          align: 'center',
          render: (value, row, key) => (
            <Link
              className="tw-link"
              to={`/org/bu/eqvaRatio/index?buId=${row.buId}&resId=${
                row.resId
              }&readOnly=view&from=/org/bu/resInfo`}
            >
              {eqvaRatioDetail.remark || '当量系数/额定当量调整记录'}
            </Link>
          ),
        },
      ]
      ...
      leftButtons: [
        ...
        },
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) => {
            // 跳转新页面
            router.push(
              `/org/bu/eqvaRatio/index?buId=${selectedRows[0].buId}&resId=${
                selectedRows[0].resId
              }&mode=edit&from=/org/bu/resInfo`
            );
          },
        },
      ],
