...
const DOMAIN = 'eqvaSalaryBuDefinition';
// 资源当量收入明细初始化
const formDataModel = {
  id: null,
  buId: null,
...
@connect(({ loading, eqvaSalaryBuDefinition, user }) => ({
  eqvaSalaryBuDefinition,
  loading: loading.effects[`${DOMAIN}/query`],
  user,
}))
class BuDefinition extends PureComponent {
  state = {
  ...
  buEqvaSubmitModal() {
    ...
    const {
      dispatch,
      user,
      eqvaSalaryBuDefinition: { buFormData },
    } = this.props;
    dispatch({
      type: `${DOMAIN}/save`,
  ...
  render() {
    const {
      ...
      eqvaSalaryBuDefinition: { dataList, total, buList, buFormData },
      user,
    } = this.props;
    const { buEqvaVisible, formData } = this.state;
