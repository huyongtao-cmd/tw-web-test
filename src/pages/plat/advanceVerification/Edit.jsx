...
const DOMAIN = 'advanceVerificationEdit'; //
 ...
 * 非差旅费用报销
 */
@connect(({ loading, dispatch, user, advanceVerificationEdit }) => ({
  loading,
  dispatch,
  user,
  advanceVerificationEdit,
}))
@Form.create({
  mapPropsToFields(props) {
    const { formData } = props.advanceVerificationEdit;
    const fields = {};
...
@mountToTab()
class advanceVerificationEdit extends React.PureComponent {
  componentDidMount() {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    const {
      advanceVerificationEdit: { formData },
    } = this.props;
    if (
      formData.reimType2 === 'PURCHASE' &&
      formData.reasonType === '04' &&
      formData.reasonId !== void 0 // 此处是判断不为undefined不用多租户改造
    ) {
      ...
      const {
        ...
        reasonType: ort,
        reasonId: orId,
      } = prevProps.advanceVerificationEdit.formData;

      if (reasonId !== orId) {
        this.fetchPhase(formData.reasonId);
      }
    }
    // console.log(formData, prevProps.advanceVerificationEdit.formData);
    ...
    const { reasonType: prt, reasonId: ori } = prevProps.advanceVerificationEdit.formData;
  ...
  render() {
    ...
    const submitting = loading.effects[param.is ? `${DOMAIN}/save` : `${DOMAIN}/create`];
            ...
            <>
              ...
              <FieldList layout="horizontal" getFieldDecorator={getFieldDecorator} col={2}>
                ...
                <Field
                  ...
                  label="申请人"
                  decorator={{
                    initialValue: formData.applyResId ? parseInt(formData.applyResId, 10) : null,
                    rules: [
                      {
                        required: true,
                        message: '请输入申请人',
...
export default advanceVerificationEdit;
