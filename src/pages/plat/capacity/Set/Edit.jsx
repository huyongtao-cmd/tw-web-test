...
import { Button, Card, Form, Input, InputNumber, Divider, Checkbox, Radio, Tooltip } from 'antd';
...
const DOMAIN = 'platCapaSetEdit';
const { Description } = DescriptionList;

@connect(({ loading, platCapaSetEdit }) => ({
...
@mountToTab()
class CapaSetEdit extends PureComponent {
  state = {
    visible: false,
    checkShow: false,
  };
  ...
  levelResolve = res => {
    const levelCapaList = res.map(row => {
      ...
      if (
        ...
        newRow.apprRes = row.apprRes.join(',');
      }
      newRow.leveldId = parseInt(row.leveldId, 10);
      newRow.id = row.id;
      if (row.isNew) {
        newRow.id = null;
      }
  ...
  handleSave = jumpPath => {
    ...
    validateFieldsAndScroll(error => {
      if (!error) {
        ...
        const newCapaSetAbilityIds = [];
        for (let i = 0; i < capaSetAbilityIds.length; i += 1) {
          if (!newCapaSetAbilityIds.includes(capaSetAbilityIds[i])) {
            newCapaSetAbilityIds.push(capaSetAbilityIds[i]);
          }
        }
        const newCapaSetLevelIds = [];
        for (let i = 0; i < capaSetLevelIds.length; i += 1) {
          if (!newCapaSetLevelIds.includes(capaSetLevelIds[i])) {
            newCapaSetLevelIds.push(capaSetLevelIds[i]);
          }
        }
        const params = {
          ...formData,
          ...
          deleteIdList: {
            capaSetAbilityIds: newCapaSetAbilityIds,
            capaSetLevelIds: newCapaSetLevelIds,
          },
        };
        ...
        if (Array.isArray(params.applyResId)) {
          params.applyResId = formData.applyResId.join(',');
        }
        const leavelIdArray = [];
  ...
  doubleCheck = () => {
    ...
    closeThenGoto(
      `/hr/capacity/doubleCheck/capaSetLaunch?id=${id}&pageType=recombination&hasLevelFlag=${hasLevelFlag}`
    );
    this.setState({
      checkShow: false,
    });
    ...
    if (apprType === 'ASSIGN_RES') {
      ...
      apprResComponent = (
        <div className={styles['approval-box']}>
          ...
          />
          <Checkbox
            checked={dataList[index].levelFlag}
            onClick={this.onCellChangedChecked2(index, 'levelFlag')}
          />
  ...
  render() {
    ...
    const { visible, checkShow, capaSearchValue } = this.state;
    ...
    newDataList.forEach(item => {
      if (item.leveldId && Array.isArray(leveDetaillList) && leveDetaillList.length > 0) {
        ...
        const levelTitle = leveDetaillList.find(levelItem => leveldId === levelItem.id)
          ? leveDetaillList.find(levelItem => leveldId === levelItem.id).name
          : '';
        const columns = {
          title: levelTitle,
    ...
    const tablePropsLevel = {
      ...
      onDeleteItems: (selectedRowKeys, selectedRows) => {
        ...
        dispatch({
          type: `${DOMAIN}/updateState`,
          payload: {
            capaSetLevelIds: capaSetLevelIds.concat(ids),
    ...
    const tablePropsAbility = {
      ...
      onDeleteItems: (selectedRowKeys, selectedRows) => {
        ...
        dispatch({
          type: `${DOMAIN}/updateState`,
          payload: {
            capaSetAbilityIds: capaSetAbilityIds.concat(ids),
    ...
    return (
      <PageHeaderWrapper title="复合能力编辑页">
        ...
        {loading ? (
          ...
          <Card
            title={
              <Title
                icon="profile"
                id="ui.menu.hr.capacity.capaSet.edite"
                defaultMessage="复合能力编辑"
              />
            }
          ...
          >
            ...
            >
              ...
              <FieldLine label="复合能力">
                ...
                >
                  <UdcSelect code="COM:JOB_TYPE1" placeholder="请选择复合能力" disabled />
                </Field>
                <Field
                  name="jobType2"
                ...
                >
                  <UdcSelect code="COM:JOB_TYPE2" placeholder="请选择复合能力" disabled />
                </Field>
              </FieldLine>
              <Field
                label="有无级别"
              ...
              <Field
                ...
                wrapperCol={{ span: 12 }}
                decorator={{
                  initialValue: formData.requestApply,
                  rules: [
                    {
                      required: true,
                      message: '请选择一项',
                ...
                <FieldLine label="首次申请审核人" required>
                  ...
                  {formData.applyStatus === 'ASSIGN_RES' ? (
                    <Field
                      ...
                      decorator={{
                        initialValue:
                          formData.applyResId && typeof formData.applyResId === 'string'
                            ? formData.applyResId.split(',').map(i => Number(i))
                            : void 0,
                        rules: [
                          {
                            required: true,
