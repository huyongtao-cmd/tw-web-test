...
const DOMAIN = 'platComputerApplyEdit';
...
@connect(({ loading, platComputerApplyEdit, dispatch }) => ({
  loading,
  ...platComputerApplyEdit,
  dispatch,
}))
@Form.create({
...
@mountToTab()
class ComputerApplyEdit extends PureComponent {
  componentDidMount() {
  ...
  handleSave = isSubmit => {
    ...
    const { page, taskId = undefined, remark = undefined } = fromQs();
    validateFieldsAndScroll((error, values) => {
      if (!error) {
        dispatch({
          type: `${DOMAIN}/save`,
          payload: { formData, page, isSubmit, taskId, params: { result: 'APPROVED', remark } },
        });
      }
    });
  };
  ...
  handleApplyResId = value => {
    ...
    if (value) {
      ...
      dispatch({
        type: `${DOMAIN}/updateForm`,
        payload: { baseCityName: null, resBuName: null, resBuId: null, monthlyAmt: null },
      });
    }
  };
...
export default ComputerApplyEdit;
