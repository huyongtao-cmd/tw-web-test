...
@connect(({ loading, dispatch, communicateMiddleFlow }) => ({
  loading,
  dispatch,
  communicateMiddleFlow,
}))
class AssessorModalOfMiddle extends React.Component {
  componentDidMount() {
    const { dispatch } = this.props;
    // 获取页面配置信息
    dispatch({
      type: `${DOMAIN}/getAssessedPageConfig`,
      payload: { pageNo: 'PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSED' },
    });
  }
  ...
  render() {
    const {
      visible,
      closeModal,
      communicateMiddleFlow: { checkAssessedData, assessedPageConfig },
    } = this.props;
    let element = null;
...
export default AssessorModalOfMiddle;
