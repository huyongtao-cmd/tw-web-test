...
import AssessorModalOfMiddle from './assessorModalOfMiddle';
import { mountToTab } from '@/layouts/routerControl';

const DOMAIN = 'communicateMiddleFlow';
const { Field } = FieldList;
const { Description } = DescriptionList;

@connect(({ loading, communicateMiddleFlow, dispatch }) => ({
  loading: loading.effects[`${DOMAIN}/queryDetail`],
  communicateMiddleFlow,
  dispatch,
}))
@Form.create({
...
@mountToTab()
class CommunicateMiddleFlow extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
    ...
    param.taskId
      ? dispatch({
          ...
          if (response) {
            ...
            let point = '';
            if (taskKey) {
              if (taskKey === 'ACC_A58_02_ASSESSED_b') {
                point = 'PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSED';
              } else if (taskKey === 'ACC_A58_03_ASSESSOR_b') {
                point = 'PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSOR';
                // 在考核人阶段是否可查看被考核人信息
                dispatch({
              ...
              } else if (taskKey === 'ACC_A58_04_HR_CFM') {
                point = 'PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_HR';
                ...
                dispatch({
                  ...
                  payload: {
                    pageNos:
                      'PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSED,PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSOR',
                  },
                });
              }
              // 获取页面配置信息
  ...
  renderAssessor = () => {
    const {
      communicateMiddleFlow: { pageConfig, formData, fieldsConfig },
  ...
  render() {
    ...
    const {
      PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSED: assessedPageConfig,
      PERFORMANCE_MIDDLE_COMMUNICATION_FLOW_ASSESSOR: assesPageConfig,
    } = pageConfigs;
    ...
    if (buttons && buttons.length === 1) {
      if (taskKey === 'ACC_A58_03_ASSESSOR_b') {
        buttons.unshift({
          type: 'button',
      ...
      if (
        taskKey === 'ACC_A58_02_ASSESSED_b' ||
        taskKey === 'ACC_A58_03_ASSESSOR_b' ||
        taskKey === 'ACC_A58_04_HR_CFM'
      ) {
        buttons.unshift({
          type: 'button',
    ...
    let element = null;
    if (taskKey && taskKey !== 'ACC_A58_01_SUBMIT_i') {
    ...
    return (
      <PageHeaderWrapper>
          ...
          <>
            <BpmWrapper
              ...
              onBtnClick={({ operation, bpmForm }) => {
                ...
                // 被考核人填写提交
                if (key === 'APPROVED' && taskKey === 'ACC_A58_02_ASSESSED_b') {
                  dispatch({
                ...
                // 考核人填写完毕后提交
                if (key === 'APPROVED' && taskKey === 'ACC_A58_03_ASSESSOR_b') {
                  dispatch({
                ...
                // hr填写完毕后提交
                if (key === 'APPROVED' && taskKey === 'ACC_A58_04_HR_CFM') {
                  dispatch({
            ...
            >
              ...
              {!taskKey || taskKey === 'ACC_A58_01_SUBMIT_i' ? (
                <div>
                  <Card
                    className="tw-card-adjust"
                    style={{ marginTop: '6px' }}
        ...
        {visible ? <AssessorModalOfMiddle visible={visible} closeModal={this.closeModal} /> : null}
      </PageHeaderWrapper>
    );
  }
}

export default CommunicateMiddleFlow;
