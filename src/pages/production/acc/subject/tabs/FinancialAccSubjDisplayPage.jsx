...
// namespace声明
const DOMAIN = 'financialAccSubjDisplayPage';

/**
 * 会计科目 综合展示页面
 */
@connect(({ loading, dispatch, financialAccSubjDisplayPage }) => ({
  loading: loading.effects[`${DOMAIN}/init`],
  dispatch,
  ...financialAccSubjDisplayPage,
}))
@Form.create({
...
class FinancialAccSubjDisplayPage extends React.PureComponent {
  componentDidMount() {
  ...
  fetchTree = async () => {
    const { data } = await outputHandle(financialAccSubjListPaging, { limit: 0 });
    return data.rows.map(item => ({ ...item, title: item.accName }));
  };
  ...
  render() {
    ...
    return (
      <PageWrapper loading={loading}>
        ...
        >
          <FormItem fieldType="BaseInput" label="编码" fieldKey="accCode" required />

          <FormItem fieldType="BaseInput" label="会计科目名称" fieldKey="accName" required />
          ...
          <FormItem fieldType="BaseInputNumber" label="科目级别" fieldKey="accLevel" required />*/}
...
export default FinancialAccSubjDisplayPage;
