...
@connect(({ loading, dispatch, invoice }) => ({
  loading,
  dispatch,
  ...invoice,
}))
    ...
    const fields = [
      {
        ...
        render: (value, row) => (
          <Link
            onClick={() => {
              const { id } = row;
              router.push(`/workTable/payAndReceive/invoice/detail?id=${id}&mode=DESCRIPTION`);
            }}
          >
            {value}
          </Link>
          ...
          extraButtons={[
            {
              ...
              cb: internalState => {
                ...
                // 跳转到新增页面
                router.push(`/workTable/payAndReceive/invoice/edit?id=${id}&mode=EDIT`);
              },
