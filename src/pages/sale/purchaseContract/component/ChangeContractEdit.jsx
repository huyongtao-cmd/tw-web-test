...
import ChangeEditableDataTable from '@/components/common/ChangeEditableDataTable';
import { UdcSelect, FileManagerEnhance, Selection } from '@/pages/gen/field';
import { fromQs, getGuid } from '@/utils/stringUtils';
import { paymentTableProps, purchaseTableProps } from '../changeConfig';
...
@mountToTab()
class ChangeContractEdit extends PureComponent {
  componentDidMount() {}
  ...
  handleSupplier = (key, data) => {
    const {
      form,
      dispatch,
      salePurchaseChangeEdit: { allAbOusArr },
    } = this.props;
    // key &&
    //   dispatch({
  ...
  purchaseInchargeResChange = value => {
    const {
      dispatch,
      salePurchaseChangeEdit: { purchaseInchargeResArr },
    } = this.props;
  ...
  linkageBu = value => {
    const {
      dispatch,
      form,
      salePurchaseChangeEdit: { purchaseBuArr },
    } = this.props;
  ...
  handleRemark = event => {
    // const { inputValue: value } = this.handleRemarkElement;

    // eslint-disable-next-line react/destructuring-assignment
    this.props.handleRemark(event.target.value);
  };

  handleAcceptanceType = value => {
    const {
      dispatch,
      salePurchaseChangeEdit: { formData, paymentList },
    } = this.props;
  ...
  //     dispatch,
  //     form,
  //     salePurchaseChangeEdit: { formData, paymentList, projectArr },
  //   } = this.props;
  ...
  taskChange = value => {
    const {
      dispatch,
      form,
      salePurchaseChangeEdit: { taskArr, formData, paymentList },
    } = this.props;
  ...
  handleUpdateForm = (value, options, name) => {
    const { dispatch, form } = this.props;
    dispatch({
      type: `${DOMAIN}/updateForm`,
      payload: {
    ...
    const mainFields = [
      ...
      >
        <Selection.UDC
          ...
          onChange={value => {
            const name = platTypeArr.find(item => item.udcVal === value);
            dispatch({
              type: `${DOMAIN}/updateForm`,
              payload: {
                platTypeDesc: name.udcTxt,
              },
            });
          }}
        />
      ...
      >
        <UdcSelect
          ...
          onChange={(value, options) => {
            this.handleUpdateForm(value, options, 'currCodeDesc');
          }}
        />
      </Field>,
    ...
    const financeFields = [
      ...
      >
        <UdcSelect
          ...
          onChange={(value, options) => {
            this.handleUpdateForm(value, options, 'payMethodName');
          }}
        />
      </Field>,
    ];
    ...
    return (
      <>
        ...
        >
          ...
          <ChangeEditableDataTable
            {...purchaseTableProps(DOMAIN, dispatch, loading, salePurchaseChangeEdit, form)}
            payState="N"
          />
        </Card>
        <Divider dashed />
        ...
        >
          ...
          <ChangeEditableDataTable
            {...paymentTableProps(DOMAIN, dispatch, loading, salePurchaseChangeEdit)}
            payState="Y"
          />
        </Card>
        ...
        <Card className="tw-card-adjust" bordered={false}>
          <FieldList
            ...
            className={style.fill}
          >
            <Input.TextArea placeholder="" rows={3} onChange={this.handleRemark} />
          </FieldList>
        </Card>
      </>
    );
...
export default ChangeContractEdit;
