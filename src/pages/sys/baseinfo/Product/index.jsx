...
import { Option, Row, Search, Tag } from 'antd';
...
const DOMAIN = 'sysProduct';

@connect(({ loading, sysProduct }) => ({
...
class BaseinfoProduct extends PureComponent {
  ...
  render() {
    const {
      loading,
      sysProduct: { searchForm, list, total },
      dispatch,
    } = this.props;
    const tableProps = {
      rowKey: 'id',
      ...
      searchBarForm: [
        ...
        },
        {
          title: '所属BU', // TODO: 国际化！！！
          dataIndex: 'buId',
          tag: <Selection source={() => selectBus()} placeholder="所属组织下拉" />,
          options: {
            initialValue: searchForm.buId,
          },
        },
        {
          title: '产品供应商', // TODO: 国际化！！！
          dataIndex: 'coopId',
          tag: <Selection source={() => selectCoop()} placeholder="请选择所属合作伙伴" />,
          options: {
            initialValue: searchForm.buId,
          },
        },
      ...
      columns: [
        {
          ...
          render: (value, row, key) => (
            <Link
              className="tw-link"
              to={`/plat/market/productdetail?canEdit=0&id=${row.id}&from=sys`}
            >
              {value}
            </Link>
          ),
      ...
      leftButtons: [
        {
          ...
          cb: (selectedRowKeys, selectedRows, queryParams) =>
            router.push(`/plat/market/productdetail?canEdit=1&from=sys`),
        },
        {
          key: 'edit',
          className: 'tw-btn-primary',
          ...
          icon: 'form',
          cb: (selectedRowKeys, selectedRows, queryParams) =>
            router.push(`/plat/market/productdetail?canEdit=1&id=${selectedRowKeys}&from=sys`),
        },
        {
          key: 'submitApproval',
          title: '提交审批',
