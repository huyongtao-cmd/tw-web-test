...
const DOMAIN = 'dataWarehouseEdit';

@connect(({ loading, dataWarehouseEdit, dispatch, user }) => ({
...
class DataWarehouseEditModal extends PureComponent {
  componentDidMount() {
  ...
  handleFields = () => {
    ...
    if (extractInfo && extractInfo.detailViews) {
      extractInfo.detailViews.forEach(column => {
        fields.push(
          <Field
            name={column.fieldColumn}
            label={column.fieldName}
            key={column.fieldColumn}
            decorator={{
              initialValue: formData[column.fieldColumn],
  ...
  handleSave = () => {
    const { form, dispatch, formData, onOk, dataTableId } = this.props;
    const copyObj = {};
    if (formData.copy) {
...
export default DataWarehouseEditModal;
