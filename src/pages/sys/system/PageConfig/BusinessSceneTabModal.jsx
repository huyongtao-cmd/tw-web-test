...
const DOMAIN = 'businessSceneTabModal';

@connect(({ loading, businessSceneTabModal, dispatch }) => ({
  loading: loading.effects[`${DOMAIN}/save`],
  ...businessSceneTabModal,
  dispatch,
}))
class BusinessSceneTabModal extends PureComponent {
  componentDidMount() {}
      ...
      columns: [
        {
          ...
          render: (value, row, index) => (
            <Input
              ...
              value={value}
              size="small"
              onChange={this.onCellChanged(index, 'tabName')}
            />
          ),
        },
        {
          ...
          render: (value, row, index) => (
            <Input
              ...
              value={value}
              size="small"
              onChange={this.onCellChanged(index, 'tabKey')}
            />
          ),
        },
        {
  ...
  handleSave = () => {
    ...
    const deleteKeys = scenePageTabEntities
      .map(item => item.id)
      .filter(item => selectedRowKeys.indexOf(item) === -1);
    const addKeys = selectedRowKeys.filter(
      item => scenePageTabEntities.map(view => view.id).indexOf(item) === -1
    );
...
export default BusinessSceneTabModal;
