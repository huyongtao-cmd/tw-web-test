...
const DOMAIN = 'systemProductDetail';
@connect(({ loading, systemProductDetail }) => ({
  systemProductDetail,
  loading,
}))
class PanelDetail extends PureComponent {
  ...
  render() {
    const { systemProductDetail, dispatch, loading } = this.props;
    const { panelType, id } = fromQs();
    return (
      <div>
        <PageHeaderWrapper title="个体赋能">
          <>{this.renderPanel({ systemProductDetail, dispatch, loading, DOMAIN, id, panelType })}</>
        </PageHeaderWrapper>
      </div>
    );
  }
