...
@connect(({ loading, SingleCreate, user }) => ({
  loading,
  SingleCreate,
  user,
}))
@Form.create()
@mountToTab()
class DetailSingleCreate extends Component {
  componentDidMount() {
    const { id } = fromQs();
  ...
  handleSave = () => {
    const {
      ...
      SingleCreate: { list, selectedRowKeys, formData: formD },
    } = this.props;
      ...
      if (!applySettleEqva || +applySettleEqva <= 0)
        ...
        const formData = {
          ...
          twEqvaSettleDEntities: Object.values(
            pickBy(value => selectedRowKeys.includes(value.key), list)
          ),
          submitted: false,
        };
  ...
  tableProps = () => {
    const {
      SingleCreate: { list, tableStatus = FILTERED_SHOW.UNFINISHED, selectedRowKeys, formData },
      loading,
      dispatch,
    } = this.props;
        ...
        onChange: (selectedKeys, selectedRows) => {
          ...
          const applySettleAmt =
            formData.settlePrice && applySettleEqva
              ? mul(applySettleEqva || 0, formData.settlePrice || 0)
              : 0;
          dispatch({
  ...
  render() {
    const {
      ...
      SingleCreate: { formData, tableStatus = FILTERED_SHOW.UNFINISHED },
      user: {
        user: { info = {} },
      },
    } = this.props;
...
export default DetailSingleCreate;
