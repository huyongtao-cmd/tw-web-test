...
// chore
const DOMAIN = 'userFeeApplySpecCreate';
...
const loop = data =>
  data.map(item => {
    if (item.children) {
      return (
        <TreeNode key={item.id} title={item.accName} value={item}>
          {!isEmpty(item.children) && loop(item.children)}
        </TreeNode>
      );
    }
    return <TreeNode key={item.id} title={item.accName} value={item} />;
  });

@connect(({ loading, userFeeApplySpecCreate, dispatch, user }) => ({
  loading,
  userFeeApplySpecCreate,
  dispatch,
  user,
}))
@Form.create({
...
@mountToTab()
class SpecCreate extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidMount() {
    const {
      dispatch,
      form,
      userFeeApplySpecCreate: { formData },
    } = this.props;
    dispatch({
      ...
      if (currentUser.user.extInfo) {
        // 资源id
        const { resId, baseBuId } = currentUser.user.extInfo;
        formData.applyBuId = baseBuId;
        form.setFieldsValue({
          applyResId: resId,
          applyBuId: baseBuId,
        });
  ...
  onCellChanged = (rowIndex, rowField) => rowFieldValue => {
    const {
      userFeeApplySpecCreate: { dataSource },
      dispatch,
      form,
    } = this.props;
  ...
  handleSave = isSubmit => {
    const {
      ...
      userFeeApplySpecCreate: { formData, dataSource, deleteList },
    } = this.props;

    if (isSubmit) {
      // 提交时校验 用途类型、费用码、是否项目相关、费用承担BU、费用总额、费用科目、费用金额
      if (!formData.usageType) {
        createMessage({ type: 'warn', description: `用途类型必填` });
        return;
      }
      // if (!formData.feeCode) {
      ...
      if (!formData.applyType) {
        createMessage({ type: 'warn', description: `是否项目相关选项必填` });
        return;
      }
      if (!formData.expenseBuId) {
        createMessage({ type: 'warn', description: `费用承担BU必填` });
        return;
      }
      if (!formData.applyAmt) {
        createMessage({ type: 'warn', description: `费用总额不能为空` });
        return;
      }
      ...
      if (dataSource.filter(v => !v.applyAmt).length) {
        createMessage({ type: 'warn', description: `费用金额必填` });
        return;
      }
    }
    // 申请人或申请人baseBU不能为空
    if (!formData.applyResId || !formData.applyBuId) {
      createMessage({ type: 'warn', description: `申请人或申请人baseBU不能为空` });
      return;
    }

    // 是否项目：选中项目时，事由号必填
    if (formData.applyType === 'PROJECT' && !formData.reasonId) {
      createMessage({ type: 'warn', description: `选中项目时，事由号必填` });
      return;
    }
    // 浮点数校验
    const re = /^[0-9]+.?[0-9]*$/;
    ...
    if (applyAmtNotNumError.length) {
      createMessage({ type: 'warn', description: `费用金额为浮点数` });
      return;
    }

    if (applyAmtError.length) {
      createMessage({ type: 'warn', description: `费用金额已超最大值999999999999` });
      return;
    }
    ...
    validateFieldsAndScroll((error, values) => {
      if (!error) {
        dispatch({
          ...
          payload: { formData, dataSource, deleteList, isSubmit },
        });
      }
    });
  };
  ...
  handleChangeExpenseBuId = value => {
    const {
      dispatch,
      form,
      userFeeApplySpecCreate: { buDataList, dataSource },
    } = this.props;
  ...
  onSelect = (rowIndex, rowField) => rowFieldValue => {
    const {
      userFeeApplySpecCreate: { dataSource, data },
      dispatch,
    } = this.props;
  ...
  render() {
    ...
    const disabledBtn =
      ...
      loading.effects['user/fetchPrincipal'] ||
      ...
      <PageHeaderWrapper>
        ...
        <Card
          ...
          title={
            <Title
              icon="profile"
              id="ui.menu.user.feeapply.specCreate"
              defaultMessage="特殊费用申请新增"
            />
          }
        >
          ...
          <FieldList layout="horizontal" getFieldDecorator={getFieldDecorator} col={2}>
            ...
            >
              <AsyncSelect source={custDataList} disabled={!projectDisabled} />
            </Field>
            ...
            <Field
              ...
              label="费用承担BU"
              decorator={{
                initialValue: formData.expenseBuId && formData.expenseBuId + '',
              }}
            >
              <AsyncSelect
                source={buDataList}
            ...
            <Field
              ...
              label="费用归属BU"
              decorator={{
                initialValue: formData.sumBuId && formData.sumBuId + '',
              }}
            >
              <AsyncSelect
                source={() => selectBus().then(resp => resp.response)}
