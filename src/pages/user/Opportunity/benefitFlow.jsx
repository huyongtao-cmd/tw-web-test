...
@mountToTab()
class BenefitFlowView extends PureComponent {
  state = {
  ...
  // 行编辑触发事件
  onBenefitCellChanged = (index, value, name) => {
    const {
      dispatch,
      opportunityBenefitDistribution: { list2 },
    } = this.props;

    const newDataSource = list2;
    newDataSource[index] = {
      ...newDataSource[index],
      [name]: value,
    };
    dispatch({
      ...
      payload: { list2: newDataSource },
    });
  };

  // 利益分配可编辑
  benefitDistributionColEdit = pageConfig => {
      ...
      render: (val, row, index) =>
        v.fieldKey === 'ruleDesc' ? (
          <Input.TextArea
            value={val}
            onChange={e => {
              this.onBenefitCellChanged(index, e.target.value, 'ruleDesc');
            }}
  ...
  render() {
    const {
      ...
      opportunityBenefitDistribution: { fieldsConfig, flowForm, pageConfig1, list2 },
    ...
    const tt1 =
      taskKey === 'ACC_A73_01_SUBMIT_i' && mode !== 'view'
        ? Array.isArray(this.benefitDistributionColEdit(pageConfig1))
          ? this.benefitDistributionColEdit(pageConfig1)
          : []
        : Array.isArray(benefitDistributionCol(pageConfig1))
          ? benefitDistributionCol(pageConfig1)
          : [];

    // loading完成之前将按钮设为禁用
    const disabledBtn = loading.effects[`${DOMAIN}/benefitFlowDetail`];
    ...
    return (
      <PageHeaderWrapper title="利益分配审批流程">
        <BpmWrapper
          ...
          onBtnClick={({ operation, bpmForm }) => {
            ...
            const { key } = operation;
            if (taskId) {
              if (taskKey === 'ACC_A73_01_SUBMIT_i') {
                dispatch({
              ...
              if (taskKey === 'ACC_A73_02_ACTIVATION_b') {
                // 第二个节点通过和退回
                if (key === 'FLOW_PASS') {
                  dispatch({
                    type: `${DOMAIN}/benefitPass`,
                    payload: {
                      payload: {
                        result: 'APPROVED',
                        taskId,
...
export default BenefitFlowView;
