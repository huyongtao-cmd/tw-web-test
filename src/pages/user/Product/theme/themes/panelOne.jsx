...
class PanelOne extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
  ...
  setReportLogModel1Props = () => {
    ...
    const {
      themeDetail: { dataSource1, reportId1 },
    } = options;
    return {
      title: '设置报表',
  ...
  setReportLogModel2Props = () => {
    ...
    const {
      themeDetail: { dataSource2, reportId2 },
    } = options;
    return {
      title: '设置报表',
  ...
  render() {
    const { options } = this.props;
    const {
      themeDetail: { processList, abilityMapList, dataSource1, dataSource2, themeItem },
    } = options;
    const {
      setThemeFlowModelProps,
      addAbilityMapModelProps,
          ...
          <div className="containRow" style={{ padding: '10px' }}>
            <Row gutter={16}>
              ...
              <Col span={8}>
                <Card
                  ...
                  className={`${styles.uploadContain} ${editIndex === 3 ? styles.borderStyle : ''}`}
                  onClick={() => handWhichBlock(3)}
                >
                  {themeItem.imgs && themeItem.imgs.length > 2 ? (
                    <img
                      alt="example"
                      width="100%"
                      height="100%"
                      src={`data:image/jpeg;base64,${themeItem.imgs[2]}`}
                    />
                  ) : (
                    <Button
                      className={`${styles.btn_center} tw-btn-primary`}
            ...
            <Row gutter={16} className={styles.distance_top}>
              ...
              <Col span={8}>
                <Card
                  title={
                    themeItem.newPanelTitle && themeItem.newPanelTitle[3]
                      ? themeItem.newPanelTitle[3]
                      : '报表'
                  }
            ...
            <Row gutter={16} className={styles.distance_top}>
              ...
              <Col span={12}>
                <Card
                  title={
                    themeItem.newPanelTitle && themeItem.newPanelTitle[4]
                      ? themeItem.newPanelTitle[4]
                      : '报表'
                  }
                  className={`${styles.flowContain} ${editIndex === 7 ? styles.borderStyle : ''}`}
                  onClick={() => handWhichBlock(7)}
                >
                  {dataSource2.length === 0 ? (
                    <Button
                      ...
                      icon="form"
                      size="large"
                      onClick={() => handSetReportLogModel2()}
                    >
                      设置报表
                    </Button>
                  ) : (
                    <ColumnarBizChart dataSource={dataSource2} />
                  )}
                </Card>
              </Col>
            </Row>
