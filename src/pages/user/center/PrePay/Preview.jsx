...
const DOMAIN = 'prePayPreview';

@connect(({ prePayPreview, user, loading }) => ({ prePayPreview, user, loading }))
@mountToTab()
class PrePayPreview extends Component {
  componentDidMount() {
  ...
  render() {
    const {
      dispatch,
      loading,
      prePayPreview: { formData, fieldsConfig: config, flowForm },
    } = this.props;
      ...
      <PageHeaderWrapper>
        <BpmWrapper
          ...
          onBtnClick={({ operation, bpmForm }) => {
            const { key } = operation;
            if (key === 'EDIT') {
              closeThenGoto(`/user/center/prePay/edit?id=${id}&remark=${bpmForm.remark}`);
        ...
        >
          ...
          >
            <DescriptionList size="large" col={2}>
              ...
              <Description term="约定付款期限">{formData.payDate}</Description>
