...
const DOMAIN = 'userExpensePayList';
...
@connect(({ loading, userExpensePayList }) => ({
  loading,
  ...userExpensePayList, // 代表与该组件相关redux的model
}))
@Form.create({
...
@mountToTab()
class PayList extends React.PureComponent {
  state = {
    details: [],
    visible: false,
  };
      ...
      columns: [
        {
          ...
          render: (value, row, index) => {
            if (row.batchNo) {
              return (
                <a href={SERVER_URL + '/api/worth/v1/acc/pay/cashier/export?id=' + row.id}>
                  导出明细
                </a>
              );
            }
  ...
  render() {
    ...
    return (
      <PageHeaderWrapper title="付款导出">
        <Card bordered={false}>
          <div className="tw-card-title">财务付款报销单</div>
