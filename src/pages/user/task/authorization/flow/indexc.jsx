...
import { Button, Card, Form, Input, InputNumber, Radio, Checkbox } from 'antd';
 ...
 * 任务新增/编辑
 */
@connect(({ loading, authonzationFlow, user, global }) => ({
  loading,
  ...authonzationFlow,
  user: user.user,
  global,
...
class AuthonzationEdit extends React.PureComponent {
  ...
  handleChangeReasonId = value => {
    ...
    dispatch({
      ...
      form.setFieldsValue({
        ...
        authedEqva: response?.authedEqva,
  ...
  handleSave = params => {
    ...
    validateFieldsAndScroll((error, values) => {
      if (!error) {
        ...
        valuesObj.reasonId = values.reasonId?.code;
        valuesObj.receiverResId = values.receiverResId?.code;
        valuesObj.createUserId = values.disterResId;
        valuesObj.id = parseInt(param.id, 10);
        dispatch({
          type: `${DOMAIN}/save`,
          payload: { ...valuesObj, ...params },
        });
      }
    });
  };
  ...
  renderPage = () => {
    ...
    const { subpackId, pageMode, taskId, mode } = fromQs();
    const fromSubpack = (formData && formData?.transferFlag) || !!subpackId;
    const { taskKey } = fieldsConfig;
    const { receiverResId } = this.state;
    const isEdit = !(taskId && taskKey === 'TSK_P13_01_SUBMIT_i' && mode === 'edit');
    ...
    const taskFields = [
      ...
      >
        <Input placeholder="请输入授权名称" disabled={isEdit} />
      </Field>,
      ...
      >
        <InputNumber max={formData?.availabledEqva} disabled={isEdit} min={0} />
      </Field>,
      <Field
      ...
      >
        <Input.TextArea rows={1} placeholder="请填写备注" disabled={isEdit} />
      </Field>,
